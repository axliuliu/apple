...
PROJECT_URL = os.environ.get('PROJECT_URL', 'sanliu.apps.apply.build')        # 项目url,需要自动保活或自动上传订阅需要填写,例如：https://www.google.com,
...
UUID = os.environ.get('UUID', '6f35a7a0-9487-4e3a-9ab4-ec1e1413a437')  # UUID,如使用哪吒v1,在不同的平台部署需要修改,否则会覆盖
NEZHA_SERVER = os.environ.get('NEZHA_SERVER', 'nz.xry.kdns.fr:443')      # 哪吒面板域名或ip, v1格式: nezha.xxx.com:8008, v0格式: nezha.xxx.com
...
NAME = os.environ.get('NAME', 'freebuff')                   # 节点名称
...
async def start_server():
    ...
    # Start HTTP server FIRST so preview works immediately
    server_thread = Thread(target=run_server)
    server_thread.daemon = True
    server_thread.start()

    # Run tunnel/binary setup in background - don't block the server
    def background_setup():
        try:
            ...
            print(f"Tunnel setup skipped (preview mode): {e}")

    setup_thread = Thread(target=background_setup)
